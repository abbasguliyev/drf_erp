    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                backup.backup_date = datetime.datetime.now()
                backup.save()
            except:
            ...
            return Response({'detail': f"Backup yerinə yetirildi: {datetime.datetime.now()}"}, status=status.HTTP_200_OK)
        except:
            traceback.print_exc()
            ...
            return Response({'detail': f"Backup zamanı xəta baş verdi: {datetime.datetime.now()}"}, status=status.HTTP_400_BAD_REQUEST)
...
def media_back_up(request):
    if request.method == "POST":
        ...
        try:
            ...
            return Response({'detail': f"Media Backup yerinə yetirildi: {datetime.datetime.now()}"}, status=status.HTTP_200_OK)
        except:
            traceback.print_exc()
            # return Response({'detail': f"Backup zamanı xəta baş verdi: {datetime.date.today().year}-{datetime.date.today().month}-{datetime.date.today().day}"}, status=status.HTTP_400_BAD_REQUEST)
            return Response({'detail': f"Backup zamanı xəta baş verdi: {datetime.datetime.now()}"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                backup.restore_date = datetime.datetime.now()
                backup.save()
            except:
                backup = BackupAndRestore.objects.create(
            ...
            return Response({'detail': f"Restore yerinə yetirildi: {datetime.datetime.now()}"}, status=status.HTTP_200_OK)
        except:
            traceback.print_exc()
            # return Response({'detail': f"Backup zamanı xəta baş verdi: {datetime.date.today().year}-{datetime.date.today().month}-{datetime.date.today().day}"}, status=status.HTTP_400_BAD_REQUEST)
            return Response({'detail': f"Backup zamanı xəta baş verdi: {datetime.datetime.now()}"}, status=status.HTTP_400_BAD_REQUEST)
