    ...
    def get(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            return self.get_paginated_response([{
                'toplam_tapsiriq_sayi': int(toplam_tapsiriq_sayi),
                'tamamlandi': int(tamamlandi),
                'icra_edilir': int(icra_edilir),
                'gecikir': int(gecikir),
...
class AdvertisementListCreateAPIView(generics.ListCreateAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if page is not None:
            serializer = self.get_serializer(page, many=True)
            return self.get_paginated_response([{
                'toplam_tapsiriq_quantityi': int(toplam_tapsiriq_quantityi),
                'data': serializer.data
            }])
        serializer = self.get_serializer(queryset, many=True)
        return Response([{
