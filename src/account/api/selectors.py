...
from account.models import EmployeeStatus, Customer, Region, EmployeeActivity
...
def user_list(*, filters=None) -> QuerySet[User]:
    ...
    qs = User.objects.select_related(
                'company', 'office', 'department', 'section', 'position', 'region', 'employee_status', 'commission',
...
def employee_activity_list(*, filters=None) -> QuerySet[EmployeeActivity]:
    filters = filters or {}
    qs = EmployeeActivity.objects.select_related('employee').all()
    return qs
