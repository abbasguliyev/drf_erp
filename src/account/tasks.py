...
from salary.api.selectors import employee_activity_history_list, salary_view_list
...
from account.api.services import create_employee_activity
from account import ACTIV, DEACTIV, FIX_COMISSION
...
@shared_task(name='create_employee_activity_task')
def create_employee_activity_task(id, is_active):
    """
    İşçi register edildiyi zaman müqavilə tarixçəsini create edən task
    """
    ...
    if user is not None:
        create_employee_activity(employee=user, status=status)
...
def create_employee_salary_view_task(id):
    ...
    if user is not None:
        ...
        if employee_salary_this_month == 0:
            if user.salary_style == FIX_COMISSION:
                sw = salary_view_service.salary_view_create(employee=user, date=f"{now.year}-{now.month}-{1}", final_salary=0)
            else:
                sw = salary_view_service.salary_view_create(employee=user, date=f"{now.year}-{now.month}-{1}", final_salary=user.salary)
