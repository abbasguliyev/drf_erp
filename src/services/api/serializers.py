...
class ServiceSerializer(DynamicFieldsCategorySerializer):
    contract = ContractSerializer(read_only=True, fields=['id','company', 'office', 'product', 'group_leader', 'manager1', 'creditor_contracts', 'contract_date', 'gifts', 'region', 'address'])
    ...
    service_creditor = UserSerializer(read_only=True, fields=['id', 'fullname', 'company', 'office'])
    operator = UserSerializer(read_only=True, fields=['id', 'fullname'])

    contract_id = serializers.PrimaryKeyRelatedField(
        queryset=contract_list(), source='contract', write_only=True, required=False, allow_null=True
    )
...
class ServiceStatistikaSerializer(DynamicFieldsCategorySerializer):
    ...
    def to_representation(self, instance):
        ...
        try:
            ...
            discount_faizi = 0
        representation['endirim_faizi'] = discount_faizi
