...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    shobe = serializers.PrimaryKeyRelatedField(queryset= Shobe.objects.all(),required=True)
    shirket = serializers.PrimaryKeyRelatedField(queryset= Shirket.objects.all(),required=True)
    ofis = serializers.PrimaryKeyRelatedField(queryset= Ofis.objects.all(),required=True)
    vezife = serializers.PrimaryKeyRelatedField(queryset= Vezifeler.objects.all(),required=True)
    ...
    class Meta:
        ...
        fields = (
            ...
            'ofis', 'vezife', 'komanda', 'user_permissions', 'groups',
    ...
    def validate(self, data):
        ...
        if data['shobe'] == None:
            raise serializers.ValidationError({"password": "Şöbəni daxil edin"})
        if data['shirket'] == None:
            raise serializers.ValidationError({"password": "Şirkəti daxil edin"})
        if data['ofis'] == None:
            raise serializers.ValidationError({"password": "Ofisi daxil edin"})
        if data['vezife'] == None:
            raise serializers.ValidationError({"password": "Vəzifə daxil edin"})
        if data['tel1'] == None:
            raise serializers.ValidationError({"password": "Ən az 1 telefon nömrəsi daxil edin"})
